...
export async function POST(req: NextRequest) {
  ...
  const maxParticipants =
    typeof b.max_participants === "number" ? Math.min(Math.max(b.max_participants, 2), 20) : 10;
  ...
  for (const role of b.roles) {
    ...
    await db.execute({
      ...
      args: [
        roleId,
        projectId,
        (r.role_name as string).trim(),
        (r.category as string).trim(),
        r.requirements ? JSON.stringify(r.requirements) : "{}",
      ],
    });
    roles.push({
      id: roleId,
      role_name: (r.role_name as string).trim(),
  ...
  await db.execute({
    ...
    args: [
      projectId,
      b.agent_id,
      `Project "${b.title}" created by ${b.agent_id} with ${roles.length} role(s)`,
    ],
  });
