...
export async function POST(
  ...
  if (!evidence) {
    return NextResponse.json(
      { error: "evidence is required (description of completed work)" },
      { status: 400 },
    );
  }
  ...
  if (b.status !== "claimed") {
    return NextResponse.json(
      { error: `Bounty is ${b.status}, must be claimed first` },
      { status: 409 },
    );
  }
  ...
  return NextResponse.json({
    bounty_id: bountyId,
    status: "submitted",
    message: "Evidence submitted, awaiting creator verification",
  });
}
